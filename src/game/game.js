...
// GAME CLASS
// ============================================================================
import { LEVELS, drawStar } from './levels.js';
...
export class Game {
  ...
  resizeGame() {
    ...
    // setupMenus is removed as React handles menus
  ...
  loadLevel(index) {
    ...
    if (this.callbacks.onLevelStart) {
      this.callbacks.onLevelStart({
        levelNum: `1-${index + 1}`,
        hint: levelData.hint,
  ...
  updateEnergyUI() {
    if (this.callbacks.onEnergyChange) {
      this.callbacks.onEnergyChange(this.player.energy, this.player.maxEnergy);
    }
  }
  ...
  triggerDeath(manual = false) {
    ...
    if (!manual) {
      ...
      if (this.callbacks.onDeathCountUpdate) {
        this.callbacks.onDeathCountUpdate(this.deathCount);
      }
      audio.playExplode();
      this.screenShake = 12;
  ...
  showLevelClearMenu() {
    ...
    this.setState('LEVEL_COMPLETE', {
      timeString,
      deathCount: this.levelDeathCount,
      rank,
      isFinalLevel,
      levelNum: this.currentLevelIndex + 1
    });
  }
  ...
  update(timestamp) {
    if (this.state === 'PLAYING' && this.player.alive) {
      ...
      if (this.callbacks.onTimeUpdate) {
        this.callbacks.onTimeUpdate(timeString);
      }
    }
